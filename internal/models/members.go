...
var MemberRels = struct {
	DistinationMemberCards string
	SenderMemberCards      string
}{
	DistinationMemberCards: "DistinationMemberCards",
	SenderMemberCards:      "SenderMemberCards",
}

// memberR is where relationships are stored.
type memberR struct {
	DistinationMemberCards CardSlice `boil:"DistinationMemberCards" json:"DistinationMemberCards" toml:"DistinationMemberCards" yaml:"DistinationMemberCards"`
	SenderMemberCards      CardSlice `boil:"SenderMemberCards" json:"SenderMemberCards" toml:"SenderMemberCards" yaml:"SenderMemberCards"`
}
...
func (r *memberR) GetDistinationMemberCards() CardSlice {
	if r == nil {
		return nil
	}
	return r.DistinationMemberCards
}

func (r *memberR) GetSenderMemberCards() CardSlice {
	if r == nil {
		return nil
	}
	return r.SenderMemberCards
}
...
// DistinationMemberCards retrieves all the card's Cards with an executor via distination_member_id column.
func (o *Member) DistinationMemberCards(mods ...qm.QueryMod) cardQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"cards\".\"distination_member_id\"=?", o.ID),
	)

	return Cards(queryMods...)
}

// SenderMemberCards retrieves all the card's Cards with an executor via sender_member_id column.
func (o *Member) SenderMemberCards(mods ...qm.QueryMod) cardQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"cards\".\"sender_member_id\"=?", o.ID),
	)

	return Cards(queryMods...)
}

// LoadDistinationMemberCards allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (memberL) LoadDistinationMemberCards(ctx context.Context, e boil.ContextExecutor, singular bool, maybeMember interface{}, mods queries.Applicator) error {
	var slice []*Member
	var object *Member
	...
	query := NewQuery(
		qm.From(`cards`),
		qm.WhereIn(`cards.distination_member_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.DistinationMemberCards = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &cardR{}
			}
			foreign.R.DistinationMember = object
		}
		return nil
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.DistinationMemberID {
				local.R.DistinationMemberCards = append(local.R.DistinationMemberCards, foreign)
				if foreign.R == nil {
					foreign.R = &cardR{}
				}
				foreign.R.DistinationMember = local
				break
			}
		}
	}
...
// LoadSenderMemberCards allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (memberL) LoadSenderMemberCards(ctx context.Context, e boil.ContextExecutor, singular bool, maybeMember interface{}, mods queries.Applicator) error {
	var slice []*Member
	var object *Member
	...
	query := NewQuery(
		qm.From(`cards`),
		qm.WhereIn(`cards.sender_member_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.SenderMemberCards = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &cardR{}
			}
			foreign.R.SenderMember = object
		}
		return nil
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.SenderMemberID {
				local.R.SenderMemberCards = append(local.R.SenderMemberCards, foreign)
				if foreign.R == nil {
					foreign.R = &cardR{}
				}
				foreign.R.SenderMember = local
				break
			}
		}
	}
...
// AddDistinationMemberCards adds the given related objects to the existing relationships
// of the member, optionally inserting them as new records.
// Appends related to o.R.DistinationMemberCards.
// Sets related.R.DistinationMember appropriately.
func (o *Member) AddDistinationMemberCards(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Card) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.DistinationMemberID = o.ID
	...
	if o.R == nil {
		o.R = &memberR{
			DistinationMemberCards: related,
		}
	} else {
		o.R.DistinationMemberCards = append(o.R.DistinationMemberCards, related...)
	}
	...
	for _, rel := range related {
		if rel.R == nil {
			...
			rel.R.DistinationMember = o
		}
	}
	return nil
}

// AddSenderMemberCards adds the given related objects to the existing relationships
// of the member, optionally inserting them as new records.
// Appends related to o.R.SenderMemberCards.
// Sets related.R.SenderMember appropriately.
func (o *Member) AddSenderMemberCards(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Card) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.SenderMemberID = o.ID
			if err = rel.Insert(ctx, exec, boil.Infer()); err != nil {
				return errors.Wrap(err, "failed to insert into foreign table")
			}
		} else {
	...
	if o.R == nil {
		o.R = &memberR{
			SenderMemberCards: related,
		}
	} else {
		o.R.SenderMemberCards = append(o.R.SenderMemberCards, related...)
	}
...
func FindMember(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*Member, error) {
	memberObj := &Member{}
...
func MemberExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
